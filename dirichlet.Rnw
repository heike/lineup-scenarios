...
But for large values of $\alpha$ the densities converge in distribution to the Binomial with parameters $K$ and probability of success $p = 1/m$, because \hh{Tritz, das ist Deines - ich bin mir sicher, dass es stimmt, aber es ist echt eklig - koenntest Du Dich darum kuemmern?}
\begin{eqnarray*}
\lim_{\alpha \to \infty} P(X_m = x) = \lim_{\alpha \to \infty} {K \choose x} \frac{B\left(x+\alpha, K-x + (m-1)\alpha \right) }{B\left(\alpha, (m-1)\alpha\right) } =

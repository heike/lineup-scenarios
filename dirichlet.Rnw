...
summary(res)
@
\caption{Comparison of sirt's estimation of $\alpha$ compared to the one described above (aka as `ours'). The true parameter value is denoted by the orange lines. It is a bit surprising, that the sirt implementation of dirichlet.mle overestimates the concentration parameter so much. }
\end{figure}

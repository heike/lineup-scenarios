...
res2 %>%
  ggplot(aes( x = value)) +
  geom_density(fill=I("grey70"), alpha=I(.5), colour=I("grey50"))  +
...
\]
For $\mu_\alpha$ and $\sigma_\alpha$ we use the estimates from the overall study as give above. However, the results are not particularly sensitive to the exact values of these hyperparameters. Figure~\ref{fig:hyper} gives an overview of the sensitivity of the results to the exact choice of hyperparameter. $\sigma_\alpha$ has a very small impact on the overall results (not shown)

\begin{figure}
<<bayes, echo=FALSE, fig.width=5, fig.height=5, out.width='.5\\linewidth'>>=
...
@
\caption{\label{fig:bayes}Scatterplot of  $p$-values with $\alpha$ estimated from a single lineup (horizontal) versus being estimated using a Bayesian approach (vertical). All $p$-values are qualitatively similar.}
\end{figure}

\begin{figure}
<<sensitivity, echo = FALSE>>=
dframe <- data.frame(expand.grid(
  mu=seq(-2.5,-1.5, by=0.1), sigma=seq(0.3, .7, by=0.05), grid=TRUE))

dframe <- rbind(dframe, data.frame(mu=distr$estimate[1], sigma=distr$estimate[2], grid=FALSE))

simul <- 1:nrow(dframe) %>% purrr::map(.f = function(j) {
  1:nrow(nsall) %>% purrr::map(.f = function(i) {
  with(nsall, bayes(x=x[i], K=K[i], m=20, mu=dframe$mu[j], sigma=dframe$sigma[j]))
  }) %>% unlist()
})
results <- data.frame(dframe, t(as.matrix(data.frame(simul))), row.names=NULL) %>% gather(key=lineup, value=pval.bayes, -mu, -sigma, -grid)
results$lineup <- with(results, reorder(lineup, pval.bayes, median))


fit <- results %>% filter(!grid)
names(fit)[5] <- "estimate"
results <- merge(results, fit[,c("lineup", "estimate")], by="lineup")
results$rel.diff <- with(results, abs(pval.bayes-estimate)/estimate)
results$abs.diff <- with(results, abs(pval.bayes-estimate))

ggplot(data=results %>% filter(grid), aes(x=lineup, y=pval.bayes)) +
  geom_point(size=1) +
  geom_line(aes(group=interaction(mu, sigma), colour = mu)) +
  geom_line(group=1, colour = "red", data=results %>% filter(!grid)) +
  ggplot2::theme_bw()
@
\caption{\label{fig:sensitive} P-values are calculated for a grid of hyperparameters $\mu_\alpha$ and $\sigma_\alpha$. The largest absolute differences in $p$-values appear for $p$-values between about 0.2 and 0.4. While the absolute differences are small for very small and very large $p$-values. At a value of about 0.125, the effect that the size of $\mu_alpha$ has on the $p$-value changes from pulling (slightly) towards  lower values as $\mu_alpha$ increases to pushing towards higher $p$-values.
}
\end{figure}
...
nulls <- merge(nulls, nsall[, c("param_value", "K", "alpha")], by="param_value")

pvals <- 1:nrow(nulls) %>% purrr::map_df(.f=function(i) {
  pval <- pval3( x = nulls$pick[i]+1/20, K=nulls$n[i]+1, alpha=nulls$alpha[i])
  pval.bayes <- bayes(x = nulls$pick[i], K=nulls$n[i],m=20)
  data.frame(pval=pval, pval.bayes=pval.bayes)
})

res.nulls <- data.frame(nulls, pvals)

qplot(data=res.nulls, geom="histogram", x=pval)
@

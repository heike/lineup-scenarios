...
elect <- raw11[grep("autismfanned2", as.character(raw11$param_value)),]
...
\frac{\Gamma(\alpha m_o)}{\Gamma(\alpha)^{m_o}} \prod_{j=1}^{m_o} p_{ij}^{\alpha-1} \\
&=& \frac{\Gamma(\alpha {m_o})^n}{\Gamma(\alpha)^{m_on}} \prod_{j=1}^n \prod_{i=1}^{m_o} p_{ij}^{\alpha-1}
\end{eqnarray*}

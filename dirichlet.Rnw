...
\begin{figure}
\centering
<<fig-alpha, echo=FALSE, dependson='pvals', out.width='.5\\linewidth'>>=
ns$example <- reorder(ns$example, ns$alpha)
qplot(example, alpha, data=ns) +
  theme_bw() + expand_limits(y=c(0,NA)) +
  geom_point(aes(y=alpha.overall), colour="orange", pch="x", size=5) +
  coord_flip() + xlab("") +
  ylab("Concentration parameter alpha")
@
\caption{\label{fig:alphas} Summary of concentration parameters $\alpha$ estimated based on individual lineups (dark circles) compared to overall estimates (colored crosses).}
\end{figure}

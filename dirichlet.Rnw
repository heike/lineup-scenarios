...
Another advantage of estimating a single concentration parameter directly, is that the estimate can be based on a single instance (such as we would have for evaluations from just one lineup). 

The package fitdistrplus \citep{fitdistrplus} unfortunately only allows fits for univariate distributions. \hh{XXX look into dirmult package}
\begin{figure}

...
<<function, echo=FALSE>>=
alpha.ml <- function(p, weight=NULL, eps=10^(-5)) {
  # matrix p
  if (is.null(dim(p))) {
    m <- length(p)
    n <- 1
  ...
  ps <- ps/rowSums(ps)
  logp <- sum(weight*rowSums(log(ps)))/(m*n)
  ml <- function(alpha) {
...
When drawing inference in the setting of a visual triangle test, we have to further consider the exact procedure with which we collect information from  observers. In this paper, we want to distinguish between  three main scenarios:
...
\end{dfn}
We will see in section~\ref{binomial} that under scenario I the visual inference distribution is the same as the Binomial distribution, ie~$V_{K, m, 1} \equiv B_{K, 1/m}$ for any concentration parameter $\alpha>0$.
...
\begin{figure}
\begin{subfigure}[t]{\textwidth}
\caption{Dirichlet samples ordered from highest to lowest values. }
...
ns <- ns %>% select(-c(alpha, dirprob, dirprob2))
...
\frac{\Gamma(\alpha m_o)}{\Gamma(\alpha)^{m_o}} \prod_{j=1}^{m_o} p_{ij}^{\alpha-1} = \frac{\Gamma(\alpha {m_o})^n}{\Gamma(\alpha)^{m_on}} \prod_{j=1}^n \prod_{i=1}^{m_o} p_{ij}^{\alpha-1}
\end{eqnarray*}
...
\]
What becomes obvious from the expression is that the concentration parameter is independent of the overall sum of the weights $w$, and only relative differences between the $w_j$ are regarded.
...
ns <- merge(ns, xpls, by="example")
@

<<tab-alpha, echo=FALSE, results='hide', dependson='pvals'>>=
xtable(ns[,-1], digits=c(0,0,0,1,4,4,4,4))
@
...
#ns$example <- reorder(ns$example, ns$alpha)
qplot(example, pval, data=ns, geom="jitter", width=0.2) +
  ggplot2::theme_bw() + expand_limits(y=c(0,NA)) +

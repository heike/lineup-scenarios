...
\]
\cgj{Sorry to be grouchy - checked this for alpha=1. Does not look like a density - perhaps need to divide by volume of simplex?}
...
Another advantage of estimating a single concentration parameter directly, is that the estimate can be based on a single instance (such as we would have for just one lineup). 
